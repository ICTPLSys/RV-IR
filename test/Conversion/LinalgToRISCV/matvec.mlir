...
func.func @main() {
    ...
    %B = arith.constant dense<[1.0, 3.0, 5.0]> : tensor<3xf64>
    
    // Create output matrix initialized to zero
    %C_init = arith.constant dense<0.0> : tensor<2xf64>
    
    // Perform matrix multiplication using linalg.matmul
    %C = linalg.matvec ins(%A, %B : tensor<2x3xf64>, tensor<3xf64>) 
                       outs(%C_init : tensor<2xf64>) -> tensor<2xf64>
    
    // Print results
    "riscv.print"(%A) : (tensor<2x3xf64>) -> ()
    "riscv.print"(%B) : (tensor<3xf64>) -> ()
    "riscv.print"(%C) : (tensor<2xf64>) -> ()
