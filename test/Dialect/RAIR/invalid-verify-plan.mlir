// RUN: torch-mlir-opt %s -split-input-file -verify-diagnostics --rair-verify-plan

rair.target @gemmini {kind = "gemmini"}

func.func @missing_plan() {
  // expected-error @+1 {{'rair.scope' op has no associated rair.plan to verify}}
  rair.scope @gemmini {
  }
  return
}
...
func.func @missing_correctness_path() {
  rair.scope @gemmini {
    %buffer, %lease = rair.reserve {space = #rair.space<spad>}
      : memref<4xf32, #rair.space<spad>>, !rair.lease
    rair.release_lease %lease, %buffer
